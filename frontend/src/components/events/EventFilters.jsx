...
export default function EventFilters({ filters, onChange, onReset }) {
  ...
  return (
    <div className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-xl mb-8 space-y-6">
      ...
      {/* Advanced Filter Row */}
      <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4 pt-4 border-t border-zinc-100 dark:border-zinc-800/80 items-center">
        {/* Location input */}
        <input
          type="text"
          ...
          className="w-full px-4 py-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-200 dark:border-zinc-700/60 text-zinc-900 dark:text-white placeholder-zinc-400 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
        ...
        {hasActiveFilters && (
          <button
            onClick={onReset}
            className="w-full py-2.5 px-4 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 text-xs font-bold transition-colors flex items-center justify-center gap-1.5"
          >
