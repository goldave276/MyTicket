...
import { BuildingIcon, ClockIcon, CheckCircleIcon, XCircleIcon, ArrowRightIcon } from '@/components/common/Icons';

export default function BecomeOrganizerPage() {
  const { user, isOrganizer, refreshUser, loading: authLoading } = useAuth();
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      router.push('/auth/login?redirect=/dashboard/become-organizer');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await reservationService.submitOrganizerRequest({
        eventType,
        documentPath: documentPath || 'document_justificatif_officiel.pdf',
        description,
      });
          ...
          {!isOrganizer && pendingRequest && (
            <div className="p-6 rounded-3xl bg-amber-500/10 border border-amber-500/20 text-amber-900 dark:text-amber-100 space-y-3">
              ...
              <div className="text-xs font-semibold">
                ...
                {new Date(pendingRequest.createdAt || pendingRequest.created_at || Date.now()).toLocaleDateString('fr-FR')}
              </div>
            </div>
          )}
          ...
          {!isOrganizer && !pendingRequest && (
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-zinc-700 dark:text-zinc-300 mb-1.5">
                    Lien / Référence du Document Officiel *
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="ex: Autorisation préfectorale / Extrait Registre du Commerce / PDF"
                    value={documentPath}
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-zinc-400">
                        {new Date(req.createdAt || req.created_at || Date.now()).toLocaleDateString('fr-FR')}
                      </p>
                    </div>
